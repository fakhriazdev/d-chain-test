    ...
    <div>
      ...
      <div className="max-w-5xl flex justify-between px-10 mb-10">
        <div className="text-darkgray">
          ...
          <p>
            {data?.data?.recipient?.city}, {data?.data?.recipient?.province}
          </p>
        ...
        <div className="text-darkgray">
          ...
          <p>
            {data?.data?.sender?.city}, {data?.data?.recipient?.province}
          </p>
      ...
      <div className="max-w-8xl gap-20 flex justify-end px-10 mb-10">
        ...
        <div>
          <p className="mb-3 text-[16px] font-bold">{`Rp. ${Number(
      ...
      <div className="grid grid-cols-2 flex max-w-8xl px-10 rounded-lg mx-auto mb-10">
        <div>
          ...
          <p className="mb-3 text-subtitle font-medium">
            {data?.data?.tenure !== 12
              ? `${data?.data?.tenure} Months`
              : "1 Year"}
          </p>
        </div>
        <div>
      ...
      {data?.data?.tenure_list_detail.map((tenures) => {
        return (
          <div key={tenures.tenure_id} className="max-w-8xl px-10 border-2 border-lightgray rounded-lg mx-auto mb-10">
            <div className="grid grid-cols-4 gap-4 py-6">
              ...
              <div>
                <p>Amount</p>
                <p className="text-[24px] text-orange font-bold">{`Rp. ${Number(
                  tenures?.amount
                ).toLocaleString("id-ID")}`}</p>
              </div>
              <div>
              ...
              <div className="m-auto">
                {tenures?.status === "ONGOING" ? (
                  <button className="bg-orange text-white py-4 px-10 rounded-lg">
                    Go to Payment
                  </button>
                ) : (
                  <button className="bg-lightgray disabled text-white py-4 px-10 rounded-lg">
                    Go to Payment
                  </button>
                )}
              </div>
