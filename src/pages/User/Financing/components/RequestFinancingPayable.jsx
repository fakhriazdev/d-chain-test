...
const RequestFinancingPayable = () => {
    ...
    const {data:financingLimit, isLoadingFinancingLimit} = useFetchFinancingLimit()
    ...
    const validationSchema = Yup.object().shape({
        ...
        checkbox: Yup.boolean().oneOf([true], 'You must declare that the information is true and correct'),
    });
    ...
    const formik = useFormik({
        ...
        onSubmit: async () => {
            try {
                const arrayData = formik.values.map(({ payment_id, amount, tenure, monthly_instalment }) => ({
                    payment_id,
                    amount,
    ...
    const handleAddDatas = () => {
        if ( formik.values[formik.values.length - 1].payment_id !== "" && (remainingLimit > 0)) {
    ...
    const handleChangeTenure = (event, index) => {
        ...
        const updatedValues = formik.values.map((value, i) =>
            i === index ? {
                ...value,
                tenure: tenure,
                monthly_instalment: parseFloat(MonthlyInstallmentCount(formik.values[index].amount, tenure))
            } : value
        );
    ...
    useEffect(() => {
        const newTotalInstallments = formik.values.reduce((total, value) => total + value.amount, 0);
            ...
            <div className="w-full rounded-lg shadow-md p-3 md:p-6 lg:p-6">
                <form onSubmit={formik.handleSubmit}>
                    {formik.values.map((datas, i) => {
                        return (
                            <div className="border-b-2 border-lightgray mb-10" key={datas?.id}>
                                <div className="mb-5 ">
                                    {datas?.id === 1 ?
                                        <p className="text-[18px] mb-3">Financing Payment {datas?.id}</p>
                                        :
                                    ...
                                    <div className="flex justify-between items-center">
                                        <p className="font-medium">{datas?.payment_id === "" ? "-" : datas?.payment_id}</p>
                                            ...
                                            {isModalVisible && (
                                                ...
                                                >
                                                    ...
                                                    <div className="relative bg-white rounded-lg shadow p-8 max-w-4xl">
                                                        {/* Modal Header */}
                                                        <div
                                                            className="flex items-center justify-between mb-5 border-gray-200">
                                                        ...
                                                        <div className="space-y-4 mb-10 w-full">
                                                            ...
                                                            <div className="overflow-y-scroll h-[500px] p-2">
                                                                {payments?.map((data) => {
                                                                    const formattedDate = data?.dueDate.split(" ")[0];
                                                                    return (
                                                                        <div
                                                                            onClick={() => handleChoosePayment(data?.transactionId, data?.amount, i)}
                                ...
                                <div className="w-full mb-10">
                                    ...
                                    <p className="text-[16px] font-medium mb-3">Rp. {datas?.amount === 0 ? "-" : datas?.amount.toLocaleString("id-ID")}</p>
                                </div>
                                <div className="flex flex-wrap justify-between mb-10">
                                    <div className="flex gap-5">
                                        ...
                                        <div className="relative text-[16px]">
                                            ...
                                            <p className="my-1.5 md:my-2 lg:my-2">
                                                {datas?.tenure}x
                                            </p>
                                        </div>
                                        ...
                                        <div className="relative text-[16px]">
                                            ...
                                            <p className="my-3">
                                                {datas.monthly_instalment === 0
                                                    ? "-"
                                                    : `${datas?.monthly_instalment.toLocaleString("id-ID")}/Mo` // Display formatted value with "IDR" and "/Mo"
                                                }
                                            </p>
                                        </div>
                                    </div>
                                ...
                                <div
                                    className="flex flex-wrap justify-center md:justify-between lg:justify-between mb-10">
                    ...
                    <div className="mb-5 mb-16">
                        ...
                        <div className="flex justify-between mb-5">
                            ...
                            <p className="font-semibold text-[16px]">{financingLimit?.remaining_limit.toLocaleString("id-ID")}</p>
                        </div>
                        <div className="flex justify-between mb-5">
                            <p>Used Limit</p>
                            <p className="font-semibold text-[16px]">{totalInstallments.toLocaleString("id-ID")}</p>
                        </div>
                        <div className="relative flex justify-between border-t-2 border-lightdark pt-5">
                            <p>Remaining Limit {remainingLimit < 0 && (
                            ...
                            * Remaining Limit must be More than or Equal 0
                            ...
                            <p className={`${
                                remainingLimit < 0 ? "font-semibold text-[24px] text-red" : "font-semibold text-[24px]"
                            }`}>
                    ...
                    <div className="mb-5 flex gap-2 mb-10">
                        <input id="default-checkbox" type="checkbox" onClick={handleCheckBox}
