...
import { paymentAction, selectPaymentAction } from "../../../../slices/paymentSlice";
...
export default function ModalChange() {
  ...
  const {
    ...
    onSubmit: async (values) => {
      ...
      console.log(data, "INI CHANGE PAYMENT METHOD");
      isCheckbox
        ? dispatch(
            paymentAction(async () => {
              const result = await paymentService.updateMethod(data);
              console.log(result);
              alert(result.message)
              window.location.reload()
            })
          )
        : console.log("Checked belum terceklis!");
    },
  });
  ...
  const handleCheckboxChange = () => {
    ...
    const methodValue = isChecked ? "AUTO_DEBIT" : "BANK_TRANSFER";
    setValues({ method: methodValue });
  };
